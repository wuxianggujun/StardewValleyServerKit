...
原因：`8080` 是 JunimoServer HTTP API，不是网页管理面板。直接用浏览器打开 API
根路径时没有 `Authorization: Bearer <API_KEY>`，所以返回 401 是正常鉴权结果。

网页管理面板默认是 `8088`：

```bash
# 有 Nginx / 1Panel / HTTPS 反向代理时使用
...
裸服务器直连模式还需要在云厂商安全组和服务器防火墙放行 `8088/tcp`，然后访问：

```text
http://<server-public-ip>:8088
```
