...
首次启动面板会在 `.env` 中生成 `ADMIN_TOKEN`，登录时从 `.env` 复制该值；终端和 systemd 日志不会打印完整令牌。面板可以查看容器健康、加入地址、端口映射、资源占用、在线玩家名称、农场角色和最近日志，也可以保存常用开服配置，管理当前 saves volume 里的存档和备份，并执行授予管理员、删除离线角色等玩家管理操作。
...
`/data/Mods`。页面会读取每个 Mod 目录下的 `manifest.json`，显示名称、
...
- `8080/tcp`：HTTP API。
- `8088/tcp`：Web 管理面板。推荐只监听 `127.0.0.1`，由 1Panel 通过 80/443 反向代理访问；明文 HTTP 监听非本机地址需要 `ADMIN_ALLOW_PUBLIC_HTTP=true`。
- `24642/udp`：游戏连接端口。
- `27015/udp`：查询端口。
