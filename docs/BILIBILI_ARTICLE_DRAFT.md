...
部署完成后，默认 / 测试存档只是服务器 saves volume 里的一个普通存档。普通用户不需要
SSH 进服务器手动删目录，直接用 Web 管理面板处理：

1. 打开 `http://<服务器公网IP>:8088`，用 `ADMIN_TOKEN` 登录。
2. 进入“存档管理”。
3. 点击“创建地图”，填写农场名、地图类型、小屋数量、人数和利润比例，确认后创建自己的正式农场。
4. 如果要切换旧存档，在“存档管理”里选择下次加载的存档，然后重启游戏服务端。
5. 如果要删除默认 / 测试存档，先确认没有玩家在线，再点该存档的“删除”。
6. 删除时输入完整存档名确认；面板会先自动备份整个 saves volume，再只删除选中的存档目录。

注意：农场地图、农场名、初始小屋数量和利润比例只影响新建农场，不会改写已有存档。
恢复备份会覆盖整个 saves volume，不是只恢复单个存档。
