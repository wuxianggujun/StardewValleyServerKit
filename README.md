...
保存配置不会热更新游戏进程。端口、人数、IP 直连等配置需要重启服务端后生效；农场地图、农场名、初始小屋数量、利润比例只对新建农场生效。要创建新地图，请先在上方配置里填好农场名、地图类型等字段，再在“存档管理”里点击“用当前配置新建地图并开服”；旧存档不会删除。要切换已有存档，请在“存档管理”里选择下次加载的存档，然后重启服务端。恢复备份会覆盖整个 saves volume，面板会在恢复前自动创建一份当前状态备份。公网服务器优先只开放 80/443 给 1Panel；除非你明确需要直连管理端口，否则不要把 `ADMIN_PORT` 暴露到公网。
