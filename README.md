...
保存配置不会热更新游戏进程。端口、人数、IP 直连等运行配置需要重启游戏服务端后生效；
农场地图、农场名、初始小屋数量、利润比例只对新建农场生效。重启 `sdv-admin.service`
只会重启网页面板，不会让 SMAPI 重新加载 Mod 或配置。

部署完成后，默认 / 测试存档只是 saves volume 里的一个普通存档，不需要 SSH 进服务器手动删：

1. 打开 Web 管理面板，进入“存档管理”。
2. 创建自己的正式农场：点击“创建地图”，填写农场名、地图类型、小屋数量、人数和利润比例后确认。
3. 切换到已有存档：在“存档管理”里选择下次加载的存档，然后重启游戏服务端。
4. 删除默认 / 测试存档：确认没有玩家在线后，点击该存档的“删除”，按页面要求输入完整存档名确认。
5. 删除前面板会自动备份整个 saves volume，然后只删除被选中的存档目录；游戏文件、Mod 和其他存档不会被删除。
6. 恢复备份会覆盖整个 saves volume，不是只恢复单个存档；恢复前面板会再备份一次当前状态。

有反向代理时公网服务器优先只开放 80/443；裸服务器直连模式需要开放 `ADMIN_PORT`，
并使用 `.env` 的 `ADMIN_TOKEN` 登录。
